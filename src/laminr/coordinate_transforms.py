...
class LinearPerDim(nn.Module):
    def __init__(self, n_dim, in_dim, out_dim, bias=True):
        ...
        self.weight = nn.Parameter(
            torch.rand(n_dim, in_dim, out_dim) * 2 * sqrt_k - sqrt_k
        )
        self.has_bias = False
        if bias:
            self.bias = nn.Parameter(
                torch.rand(n_dim, 1, out_dim) * 2 * sqrt_k - sqrt_k
            )
            self.has_bias = True
...
class CouplingLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.untouched_half == "first":
            z1 = x1
            positive_scale = torch.exp(self.s(x1)) if self.affine else 1.0
            ...
            positive_scale = torch.exp(self.s(x2)) if self.affine else 1.0
    ...
    def inverse(self, z):
        ...
        if self.untouched_half == "first":
            x1 = z1
            positive_scale = torch.exp(self.s(z1)) if self.affine else 1.0
            # positive_scale = F.elu(self.s(z1)) + 1 + self.eps if self.affine else 1.
            x2 = (z2 - self.t(z1)) / positive_scale
        elif self.untouched_half == "second":
            positive_scale = torch.exp(self.s(z2)) if self.affine else 1.0
...
class AffineTransformOld(nn.Module):
    def __init__(self, n_neurons, in_dim=2, stochastic=False, init_noise_scale=0.1):
        super().__init__()
        ...
        if stochastic:
            ...
            self._As_noise_scale = nn.Parameter(
                torch.log(torch.ones(n_neurons, in_dim, in_dim) * init_noise_scale)
            )
            self._translation_noise_scale = nn.Parameter(
                torch.log(torch.ones(n_neurons, 1, in_dim) * init_noise_scale)
            )
    ...
    def forward(self, x, stochastic=None):
        ...
        if stochastic:
            ...
            translation = (
                self.translation
                + torch.randn_like(self.translation) * self.translation_noise_scale
            )
        else:
            As = self.As
...
class AffineTransform(nn.Module):
    def __init__(
        ...
        )  # Rotation angles for each neuron

        if allow_scale:
            # Learnable diagonal elements of the scaling matrix
            self.scalings = nn.Parameter(
                torch.ones(n_neurons, in_dim)
        ...
        if allow_shear:
            ...
            self.shears = nn.Parameter(
                torch.zeros(n_neurons, in_dim)
            )  # Only learn the shear terms
        else:
    ...
    @property
    def angles_noise_scale(self):
        return torch.exp(self._angles_noise_scale)

    @property
    def scalings_noise_scale(self):
        return torch.exp(self._scalings_noise_scale)

    @property
    def shears_noise_scale(self):
        return torch.exp(self._shears_noise_scale)
    ...
    def shear_matrices(self):
        return torch.stack(
            [
                torch.stack(
                    [torch.ones_like(self.shears[:, 0]), self.shears[:, 0]], dim=-1
                ),
                torch.stack(
                    [self.shears[:, 1], torch.ones_like(self.shears[:, 1])], dim=-1
                ),
            ],
            dim=-2,
        )
